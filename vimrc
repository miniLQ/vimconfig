...
" gocode go语言代码补全插件
"Plug 'mdempsky/gocode', { 'rtp': 'vim', 'do': '~/.vim/pack/vendor/start/gocode/vim/symlink.sh' }
...
"endif
"if isdirectory(expand("~/.vim/pack/vendor/start/vim-colors-solarized"))
...
" molokai插件
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/molokai"))
	set background=dark
	colorscheme molokai
endif
...
if isdirectory(expand("~/.vim/pack/vendor/start/vim-airline"))

	if isdirectory(expand("~/.vim/pack/vendor/start/vim-airline-themes/"))
		"~/.vim/pack/vendor/start/vim-airline-themes/autoload/airline/路径下可以查看配色
...
" 配置lightline，美化状态栏
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/lightline.vim"))
	let g:lightline = {
				\ 'colorscheme': 'molokai',
				\ }
...
" gutentags 插件设置
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/vim-gutentags"))
...
"if isdirectory(expand("~/.vim/pack/vendor/start/vim-markdown/"))
...
if isdirectory(expand("~/.vim/pack/vendor/start/vim_current_word"))
...
" 配置rainbow插件，显示彩色的括号
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/rainbow"))
...
" 配置tagbar插件
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/tagbar"))
	let g:tagbar_width = 40
	let g:tagbar_right = 1
	let g:tagbar_autoclose = 0
	let g:tagbar_sort = 0
	let g:tagbar_ctags_bin='/usr/bin/ctags'
    "autocmd BufReadPost,BufNewFile *.sh,*.cpp,*.c,*.h,*.hpp,*.cc,*.cxx,*.java call tagbar#autoopen()　 "在某些情况下自动打开tagbar
...
if isdirectory(expand("~/.vim/pack/vendor/start/nerdtree"))
	...
	let g:NERDTreeWinPos="left"   " 在右侧打NERDTree
	let g:NERDTreeWinSize=40      " 宽度设置为25个字符
...
" 配置CrtlP插件
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/ctrlp.vim"))
	let g:ctrlp_map = '<c-p>'
...
if g:dst_complete_mode is# 'old' && isdirectory(expand("~/.vim/pack/vendor/start/OmniCppComplete"))
...
if isdirectory(expand("~/.vim/pack/vendor/start/ultisnips")) && has('python3')
	...
	if isdirectory(expand("~/.vim/pack/vendor/start/vim-snippets/"))
...
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/auto-pairs"))
endif

" --------------------------------------------------------}}}2
" 配置indentLine插件，显示缩进提示符
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/indentLine"))
...
" 配置YouCompleteMe插件
" --------------------------------------------------------{{{2
if g:dst_complete_mode is# 'YouCompleteMe' && isdirectory(expand("~/.vim/pack/vendor/start/YouCompleteMe"))
	" Python Semantic Completion
	"let g:ycm_python_binary_path = '/usr/bin/python3'

	" 寻找全局配置文件
	"let g:ycm_global_ycm_extra_conf = '~/.vim/bundle/YouCompleteMe/cpp/ycm/.ycm_extra_conf.py'
	"let g:ycm_global_ycm_extra_conf = '~/.vim/pack/vendor/start/YouCompleteMe/third_party/ycmd/cpp/ycm/.ycm_extra_conf.py'
...
" 配置Onedark插件
" --------------------------------------------------------{{{2
"if isdirectory(expand("~/.vim/pack/vendor/start/onedark/"))
"	let g:lightline = {
"	\ 'colorstheme':'Onedark',
"    \}
"endif
...
" 配置NERDCommenter插件
" --------------------------------------------------------{{{2
if isdirectory(expand("~/.vim/pack/vendor/start/NERDCommenter/"))
...
" <F11> 以绝对路径在当前目录下，生成cscope数据库文件<目录名.out>
...
nnoremap <silent> <F6> :call RunShell("Generate cscope.out", "bash ~/.vim/shell/create_cscope.sh")<cr>
"nnoremap <silent> <F7> :call RunShell("Generate cscope.out", "cscope -Rb")<cr>:cs add cscope.out<cr>
nnoremap <silent> <F8> :call RunShell("Generate tags", "ctags -R --c++-kinds=+p --fields=+iaS --extra=+q .")<cr>
...
if isdirectory(expand("~/.vim/pack/vendor/start/vim-choosewin"))
	" invoke with '-'
	nmap  -  <Plug>(choosewin)
...
autocmd BufNewFile *.c,*.cpp,*.sh,*.py,*.java,*.go exec ":call SetTitle()"
